...
# Buckle up, because this is about to get super math fun.  SVM relies on being able to 
# split up your dataset into two (or more potentially) distinct groups.  In order to do
# this you need something with which to separate the points.  Enter the first real weird
# term we have seen, the "hyperplane".  The hyperplane is your boundary line.  And the
# hyperplane exists in however many dimensions you need.  Are you plotting in 2D?  
# Just for fun, we call that R^2, and your hyperplane would just be a line.  Are you 
# in 3D?  Well that is R^3 and your hyperplane is a normal old plane (think a sheet of paper).
# Are you in 4D?  That is R^4 and it makes my head hurt to picture that 'plane' but
# I have had to mathematically prove it exists!  This generalizes to however many
# variables you want.  In our case it is 7 variables, so R^7, casually a seven 
# dimensional plane.  Fortunately, R will do a lot of the heavy work for us.

# Real world example:  I quite enjoy baking.  It is relaxing, fun, and very very
# satisfying to kind of start new each time.  A small debate I have seen come up is
# the difference between muffins and cupcakes.  Are cupcakes just muffins with icing?
# Are muffins cupcakes that have stuff in them?  Quite an odd question, right?
# There is clearly something different... but how do we qualify the difference?
# A quick search shows that cupcakes have a higher sugar count, and a much higher
# butter count.  This makes perfect sense, right?  Cupcakes are always much sweeter
# and typically much less dense.  A brief aside, allow me to nerd-out on baking.  
# The butter melts and infiltrates the batter.  That is what makes cupcakes so moist
# and 'airy'.  Well, one of the things anyway!  So if we were to plot recipes with 
# butter as x and sugar as y we might find muffins huddled near the origin.  On the other
# hand high butter and high sugar means that cupcakes would have some distance from 
# the origin.  We could draw a line to separate!  That is our hyperplane.

# There are more details.  Of course there are!  This is complicated and weird.
# BUT, that is the general idea.  And it will work for what we need.  So, what is 
# going to happen here?  We are going to 'plot' the variables, but in 7 dimensions.
# SVM is going to look to find a plane to split into accepted and not-accepted.
# Once SVM has that plane, it will 'plot' the test variables, and depending on which
# region the points fall in we will get accepted or not-accepted.  Easy enough!

rm(classifier, cm)

library(e1071) #Yes it is still loaded.  But this is where SVM lives, for reference.

classifier = svm(formula = Admitted.Y.N ~., 
                 data = training_set,
...
cm = table(test_set[, 5], y_preds)

# That is a bit of a problem right?  Again, we have a model that 'learned' that 
# the most accurate thing it can do is just predict all accepted.  We probably needed
# more data.  SVM is (I promise) a very powerful and fairly easy to use technique.
# We will undoubtedly revisit SVM in future walk-throughs!
#   
# Well, that is what I have.  Seven passes, roughly 1000 or so lines of code, not too bad
# right?  I hope you have enjoyed R as much as I have.  Also, hopefully you have
# gotten some things out of these lessons.  I am always looking for feedback and
# am more than happy to discuss things further!  I can hardly wait to start something
# new and fun.
